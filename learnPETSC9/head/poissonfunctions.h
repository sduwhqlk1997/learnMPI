...
PetscReal (*f_rhs)(PetscReal x, PetscReal y, PetscReal z, void *ctx); // 定义了一个函数指针，其中第四个参数，类型为 void*，是一个通用指针，通常用于传递上下文信息或额外数据。
...
PetscErrorCode Poisson1DFunctionLocal(DMDALocalInfo *info, PetscReal *au, PetscReal *aF, PoissonCtx *user);
...
PetscErrorCode Poisson1DJacobianLocal(DMDALocalInfo *info, PetscReal *au, Mat J, Mat Jpre, PoissonCtx *user);
...
PetscErrorCode Poisson2DJacobianLocal(DMDALocalInfo *info, PetscReal **au, Mat J, Mat Jpre, PoissonCtx *user);
...
PetscErrorCode Poisson3DJacobianLocal(DMDALocalInfo *info, PetscReal ***au, Mat J, Mat Jpre, PoissonCtx *user);
